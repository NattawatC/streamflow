...
export const getUserBanks = async (userId: string | undefined) => {
  ...
  // Step 1: Get estateId using userId
  const { estateId, error: estateError } = await getUserEstateId(userId)
  if (estateError || !estateId) {
    console.error("Error getting estateId:", estateError)
    return null
  }

  // Step 2: Use estateId to get bank_info
...
export const getAllElectricityMeterById = async (
